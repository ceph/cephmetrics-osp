...
# Open Grafana's port
openstack security group rule create --ingress --protocol tcp \
                                     --dist-port 3000 $SEC_GROUP_NAME
# Open Prometheus' port
openstack security group rule create --ingress --protocol tcp \
                                     --dist-port 9090 $SEC_GROUP_NAME
# Open node_exporter's port
openstack security group rule create --ingress --protocol tcp \
                                     --dist-port 9100 $SEC_GROUP_NAME
# Allow pings
...
openstack server create --flavor $FLAVOR_NAME --image $IMAGE_NAME \
                        ...
                        --nic net-id=$PRIV_NET_ID --security-group $SEC_GROUP_NAME \
                        --wait $SERVER_NAME

openstack floating ip create --floating-ip-address $FLOAT_IP $PUB_NET_NAME
openstack server add floating ip $SERVER_NAME $FLOAT_IP
